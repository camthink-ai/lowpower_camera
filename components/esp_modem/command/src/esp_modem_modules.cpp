...
command_result EG91X::set_data_mode()
{
    // Use PDP context id configured in dce_config (default 1)
    int context_id = pdp ? static_cast<int>(pdp->context_id) : 1;
    return dce_commands::set_data_mode_eg91x(dte.get(), context_id);
}
