 ...
 */
 ...
 template <typename T> command_result generic_get_string(CommandableIf *t, const std::string &command, T &output, uint32_t timeout_ms)
 {
     ...
     return t->command(command, [&](uint8_t *data, size_t len) {
         // Log raw response chunk for easier debugging (similar to v1.0.1 behaviour)
         size_t pos = 0;
         std::string_view response((char *)data, len);
         ESP_LOGI(TAG, "len:%d Response: %.*s", (int)len, static_cast<int>(response.size()), response.data());
 ...
 // EG91x (EG915/EG912) / Verizon: dial specific PDP context using ATD*99***<cid>#.
 // For example, context_id = 3 -> ATD*99***3# (used for Verizon's third PDP context).
 command_result set_data_mode_eg91x(CommandableIf *t, int context_id)
 {
     ESP_LOGV(TAG, "%s", __func__);
